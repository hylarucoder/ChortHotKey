...
WinGet, hwnd, ID,  Select File - t.ahk	;获取测试窗口的句柄
RemoteBuf_Open(bufID, hwnd, 256) 			;在测试窗口的进程中分配内存
bufAdr  := RemoteBuf_Get(bufID)				;获取分配的内存在测试进程中的地址
SendMessage, 1024+102, 256, bufAdr,, ahk_id %hwnd%	;发送消息,获取路径
VarSetCapacity(txt, 256, 0)						;分配临时变量
RemoteBuf_Read(bufID, txt, 256 )				;读取目标进程的内存到临时变量中
msgbox % txt											;输出获取到的路径
Return
